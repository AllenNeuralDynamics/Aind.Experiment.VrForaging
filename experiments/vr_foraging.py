"""VR Foraging experiments and recovery workflow."""
...
async def _run_vr_foraging_experiment(launcher: Launcher, *, with_fip: bool) -> None:
    ...
    except Exception as e:  # noqa: BLE001 -- water logging is best-effort after a completed session
...
@experiment(name="recover-session", order=4)
        ...
        while session_end_time is None:
            try:
                s = launcher.frontend.prompt_text(
                    ui.TextRequest(
                        label=(
                            "Enter the session end time in ISO format (YYYY-MM-DDTHH:MM:SSz), "
